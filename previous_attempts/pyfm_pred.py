...
def recommendation_algorithm(directory_path, selected_date, show_all=True):
    ...
    try:
        train.to_csv(directory_path + '/train2405.csv', sep=',')
        test.to_csv(directory_path + '/test2405.csv', sep=',')
    ...
    '''if show_all == False:
        ...
        for store in stores['Store_Number']:
            print(store)
            for product in products['Material_ID']:
                train_mat_data = [{'store': store, 'product': product}]
                train_mat = v.transform(train_mat_data)
                pred[(store, product)] = best_fm.predict(train_mat)[0]
        print(len(pred))
        print(pred)
        return pred'''
...
print(recommendation_algorithm(
    'brouillon/wetransfer_projet-ia_2023-02-06_1410', 2, show_all=True))
